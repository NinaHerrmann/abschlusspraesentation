...
* Eingriff an der Stelle, an der das aktuelle Token geprüft wird
* Wenn das aktuelle Token abgelaufen, fordere neues mit Refresh an

---

Problem: **Authorization Header** mit Client Credentials bei Anfrage eines Tokens wird nicht erzeugt.

* Anfragen an ownCloud server mittels cURL
* **post**-Methode setzt einen Basic Authentiation Header,<br>falls ein Authorization Code angefordert werden soll
...
* **GET**-Request: .../authorize Schnittstelle
* **clientid:** Client ID aus der OAuth 2.0 App
* **resonse_type:** code (für Authorization Code)
* **redirect_uri:** /admin/oauth2callback -> Wiederherstellung der Session
...
* **POST**-Request: .../token Schnittstelle
...
* **WebDAV-Methode** (z.B. MKCOL) an die ownCloud WebDAV Schnittstelle
    * /remote.php/webdav
* Die Anfrage bleibt gleich bis auf Authentocation Header
* Statt Basic Authentication mit username/password, Bearer Authentication mit Access Token